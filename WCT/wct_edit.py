...
import numpy as np
import time
from model_edit import WCTModel
...
class WCT(object):
    ...
    def __init__(self, input_checkpoint, relu_targets, vgg_path, device='/gpu:0',
        ...
        with graph.device(device):
            ...
            relu_vars = tf.trainable_variables()
            saver = tf.train.Saver(var_list=relu_vars)
            #saver.restore(self.sess, r"C:\Coding\Python\WCT_tf_final_seg\ckpt\model.ckpt-12801")
            saver.restore(self.sess, input_checkpoint)

            self.content_input = tf.get_default_graph().get_tensor_by_name('encoder_decoder_relu3_1/vgg_encoder/content_imgs:0')
            self.decoded_output = tf.get_default_graph().get_tensor_by_name('encoder_decoder_relu3_1/decoder_relu3_1/relu3_1_6/BiasAdd:0')
    ...
    def preprocess(image):
        ...
        return np.concatenate([image[...,:-1] / 255, image[...,-1:]], axis=-1)
