...
from utils import get_files, get_img_random_crop, get_img_seg_random_crop
...
def batch_gen(folder, batch_shape):
    ...
    #seg_files = np.asarray(get_files(folder.replace("2017", "2017_seg")))
    ...
    while True:
        ...
        while idx < batch_shape[0]:  # Build batch sample by sample
            try:
                ...
                f_seg = f.replace(r"2017", r"2017_seg").replace("jpg", "png")

                X_batch[idx] = get_img_seg_random_crop(f, f_seg, resize=512, crop=256).astype(np.float32)
...
def train():
    ...
    batch_shape = (args.batch_size,256,256,5)
    ...
    with tf.Graph().as_default():
        ...
        queue = tf.FIFOQueue(capacity=100, dtypes=[tf.float32, tf.float32], shapes=[[256,256,5], [256,256,5]])
