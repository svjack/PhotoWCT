...
def unpooling(t2, h, w, c):
    input, indices = t2
    sliced = tf.reshape(tf.gather(tf.reshape(input, [-1]), tf.reshape(indices, [-1])), [-1, h, w, c])

    resize_pool = tf.image.resize_images(sliced, size=(h * 2, w * 2), method=1,)
    return resize_pool
...
def Conv2DReflect(x ,lambda_name, *args, **kwargs):
    return tf.layers.conv2d(inputs=pad_reflect(x) ,name=lambda_name, *args, **kwargs)
